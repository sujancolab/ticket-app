        ...
        <ion-card>
          <ion-card-content>
            ...
            <ion-item class="input-item">
              ...
              <ion-select v-model="form.site_id" interface="action-sheet" @ionChange="onSiteChange">
            ...
            <ion-item class="input-item">
              ...
              <ion-select v-model="form.category_id" interface="action-sheet" @ionChange="onCategoryChange">
            ...
            <ion-item class="input-item">
              ...
              <ion-select v-model="form.sub_category_id" interface="action-sheet" @ionChange="onSubCategoryChange">
                <ion-select-option v-for="subCategory in sub_categories" :key="subCategory.id" :value="subCategory.id">
                  {{ subCategory.name }}
                </ion-select-option>
              </ion-select>
            ...
            <ion-item class="input-item">
              ...
              <ion-select v-model="form.machine_id" interface="action-sheet" @ionChange="onMachineChange">
            ...
            <ion-item class="input-item">
              ...
              <ion-input v-model="form.problem_description" type="text"
                placeholder="Enter Machine's problem..."></ion-input>
            </ion-item>

            <!-- Complaint Nature -->
            <ion-item class="input-item">
              <ion-label position="stacked">Complaint Nature <span class="required">*</span></ion-label>
              <ion-select v-model="form.complaint_nature" interface="action-sheet">
            ...
            <ion-item class="input-item">
              ...
              <ion-select v-model="form.maintenance_provider" interface="action-sheet"
              ...
              <ion-note slot="error" v-if="submitted && !form.maintenance_provider">Maintenance Provider is
                required</ion-note>
            </ion-item>
            ...
            <ion-item class="input-item" v-if="form.maintenance_provider === 'vendor'">
              ...
              <ion-select v-model="form.vendor_id" interface="action-sheet" multiple>
                <ion-select-option v-for="user in users" :key="user.id" :value="user.id">
                  {{ user.name }}
                </ion-select-option>
              </ion-select>
              <ion-note slot="error"
                v-if="submitted && form.maintenance_provider === 'vendor' && !form.vendor_id.length">
                Vendor is required
              </ion-note>
            </ion-item>
            ...
            <ion-item class="input-item" v-if="form.maintenance_provider === 'user'">
              ...
              <ion-select v-model="form.user_id" interface="action-sheet">
                <ion-select-option v-for="user in users" :key="user.id" :value="user.id">
                  {{ user.name }}
                </ion-select-option>
              </ion-select>
                  ...
                  <ion-col size="6">
                    <ion-item lines="none">
                      <ion-radio value="Free Service"
                        :disabled="form.id != '' || (selected_machine && (selected_machine.completed_free_service >= selected_machine.free_service_no))"></ion-radio>
            ...
            <ion-item class="input-item" v-if="form.service_type === 'Free Service'">
              ...
              <ion-input v-model="form.free_service_no" type="text" readonly></ion-input>
            ...
            <ion-item class="input-item">
              ...
              <ion-select v-model="form.ticket_type" interface="action-sheet"
            ...
            <ion-item class="input-item">
              ...
              <ion-select v-model="form.status_during_complaint" interface="action-sheet">
            ...
            <ion-item class="input-item">
              ...
              <ion-datetime v-model="form.likely_date" presentation="date" :min="new Date().toISOString()"
                :disabled="form.id != '' && form.likely_date != '' && pop_ticket && pop_ticket.status == 'Open'"></ion-datetime>
            ...
            <ion-item class="input-item"
              ...
              <ion-datetime v-model="form.actual_fault_date" presentation="date-time" :max="new Date().toISOString()"
                :disabled="form.id != '' && form.actual_fault_date != '' && pop_ticket && pop_ticket.status == 'Open'"></ion-datetime>
            ...
            <ion-item class="input-item" v-if="expired_likely_date && form.id">
              ...
              <ion-datetime v-model="form.reversion_date" presentation="date" :min="new Date().toISOString()"
                :disabled="form.id != '' && pop_ticket.reversion_date != '' && pop_ticket.reversion_date != 'null' && pop_ticket.reversion_date != null"></ion-datetime>
            </ion-item>

            <!-- Reason for Date Revision -->
            <ion-item class="input-item" v-if="expired_likely_date && form.id">
              <ion-label position="stacked">Reason for Date Revision</ion-label>
              <ion-input v-model="form.reversion_reason"
                :disabled="form.id != '' && pop_ticket.reversion_date != '' && pop_ticket.reversion_date != 'null' && pop_ticket.reversion_date != null"></ion-input>
            </ion-item>
            ...
            <ion-card>
              ...
              <ion-card-content>
                ...
                <ion-item class="input-item">
                  ...
                  <ion-input v-model="form.estimated_material_cost" type="number"></ion-input>
                </ion-item>

                <!-- Estimated Material Cost File -->
                <ion-item class="input-item">
                  <ion-label position="stacked">Estimated Material Cost File</ion-label>
                  <input type="file" @change="uploadMaterialCost" multiple ref="filematerial" />
                </ion-item>

                <!-- Estimated Service Cost -->
                <ion-item class="input-item">
                  <ion-label position="stacked">Estimated Service Cost</ion-label>
                  <ion-input v-model="form.estimated_service_cost" type="number"></ion-input>
                </ion-item>

                <!-- Estimated Service Cost File -->
                <ion-item class="input-item">
                  <ion-label position="stacked">Estimated Service Cost File</ion-label>
                  <input type="file" @change="uploadServiceCost" multiple ref="fileservice" />
                </ion-item>
              </ion-card-content>
            </ion-card>
            ...
            <ion-card v-if="form.id">
              <ion-card-header>
                ...
                <ion-button @click="AddStage" size="small"
              ...
              <ion-card-content>
                <ion-grid>
                  ...
                  <ion-row v-for="(field, x) in form.stages" :key="'s' + x" class="stage-row">
                    <ion-col size="3">
                      <ion-select v-model="field.stage_no" :disabled="x < form.stages.length - 1">
                    ...
                    <ion-col size="3">
                      <ion-datetime v-model="field.stage_date" presentation="date" :min="last_stage_date"
                        :max="new Date().toISOString()"></ion-datetime>
                    </ion-col>
                    <ion-col size="5">
                      <ion-input v-model="field.description" placeholder="Enter Description"></ion-input>
                    </ion-col>
                    <ion-col size="1">
              ...
              <ion-card-content>
                <ion-grid>
                  ...
                  <ion-row v-for="(field, x) in form.problems" :key="'ap' + x" class="problem-row">
                    <ion-col size="5">
                      <ion-input v-model="field.problem" placeholder="Enter Machine's problem..."></ion-input>
                    </ion-col>
                    <ion-col size="3">
                      <ion-input v-model="field.estimated_material_cost" type="number"></ion-input>
                    </ion-col>
                    <ion-col size="3">
                      <ion-input v-model="field.estimated_service_cost" type="number"></ion-input>
                    </ion-col>
                    <ion-col size="1">
                      <ion-button @click="form.problems.splice(x, 1)" fill="clear">
                        <ion-icon :icon="closeCircle" slot="icon-only"></ion-icon>
                      </ion-button>
                    </ion-col>
                  </ion-row>
            ...
            <!-- Submit Button -->
            <ion-button expand="block" type="submit" class="ion-margin-top" :disabled="isSaving">
...
const api = axios.create({
  baseURL: 'https://rmm.tech-trico.com', // Replace with your API base URL
  timeout: 10000,
});
